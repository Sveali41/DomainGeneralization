...
from modelBased.common.utils import normalize
...
def data_collect(cfg, env):
    obs, obs_next, act,rew, done = run_env(env, cfg.env)
    if cfg.env.collect.data_train == None:
...
def collect(cfg: DictConfig):
    path = Paths()
    env = FullyObsWrapper(CustomMiniGridEnv(txt_file_path=path.LEVEL_FILE_Rmax2, custom_mission="Find the key "
                                                                                      "and open the "
                                                                                      "door.",
                                        max_steps=2000, render_mode=None))
    data_collect(cfg.env, env)
